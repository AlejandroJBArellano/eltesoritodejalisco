...
export function getTenantSlugFromHost(host: string | null): string {
  ...
  if (hostname.endsWith(".trykittn.com") && !hostname.endsWith("admin.trykittn.com")) {
    const parts = hostname.split(".");
    if (parts.length >= 3) {
      return parts[0];
    }
...
export const getTenantBySlug = cache(async (slug: string): Promise<TenantContextType | null> => {
  try {
