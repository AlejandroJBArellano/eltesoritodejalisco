...
export async function pauseTask(executionId: string) {
  ...
  const newlyPausedSeconds = Math.floor(
    (now.getTime() - lastResumedAt.getTime()) / 1000,
  );
...
export async function completeTask(executionId: string, photoUrl?: string) {
  ...
  const now = new Date();
  const finalPausedSeconds = current.paused_seconds || 0;

  const startTime = new Date(current.start_time);
  const grossDurationSeconds = Math.floor(
    (now.getTime() - startTime.getTime()) / 1000,
  );
  const netDurationMinutes = Math.floor(
    (grossDurationSeconds - finalPausedSeconds) / 60,
  );
    ...
    return {
      ...
      avgDurationMinutes:
        item.completedCount > 0
          ? Math.round(item.totalDuration / item.completedCount)
          : 0,
    };
  });
}
