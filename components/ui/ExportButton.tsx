...
export function ExportButton<T>({
  ...
  const variantStyles = {
    ...
    primary:
      "bg-primary text-dark hover:bg-secondary shadow-xs focus-visible:ring-primary/40",
    ghost:
    ...
    outline:
      "bg-transparent border border-border text-text-light hover:bg-white/5 hover:text-white focus-visible:ring-primary/40",
  };
  ...
  return (
    ...
    >
      <button
        ...
        className={`flex items-center gap-2 rounded-lg px-3 py-1.5 text-xs font-semibold uppercase tracking-wider transition-all duration-150 cursor-pointer select-none active:scale-95 disabled:opacity-40 disabled:cursor-not-allowed disabled:active:scale-100 outline-none focus-visible:ring-1 ${
      ...
      {isOpen && (
        <div
          ...
          className={`absolute z-50 mt-1.5 w-56 rounded-xl border border-border bg-card p-1.5 shadow-xl backdrop-blur-md animate-in fade-in zoom-in-95 duration-100 ${
        ...
        >
          ...
          >
            <div className="rounded-md bg-emerald-500/10 p-1.5 text-emerald-400 group-hover:bg-emerald-500/20 transition-colors shrink-0">
              <FileSpreadsheet className="h-4 w-4" />
            </div>
            <div>
              <p className="text-xs font-semibold text-text-light group-hover:text-white">
                Excel (.xls)
              </p>
