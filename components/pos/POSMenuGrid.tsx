...
> = {
  ...
  OTROS: {
    label: "Otros",
    color: "#E0E0E0",
    badgeBg: "bg-dark/40 text-text-light/80 border-border",
    text: "#E4E4E7",
  },
};
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-border pb-3.5">
        ...
        <div className="relative min-w-55">
          ...
          <input
            ...
            className="w-full rounded-lg border border-border bg-dark/40 pl-8 pr-7 py-1.5 text-xs text-text-light outline-none focus:border-primary focus:ring-1 focus:ring-primary/30 transition-all placeholder:text-text-light/30"
          />
          {searchQuery && (
            <button
              type="button"
  ...
  return (
    ...
    >
      ...
      <div className="mt-2 flex items-center justify-between w-full gap-1 flex-wrap">
        <span className="rounded-md bg-dark/40 border border-border px-2 py-0.5 text-xs font-mono font-bold text-text-light tabular-nums">
          ${m.price.toFixed(2)}
        </span>
