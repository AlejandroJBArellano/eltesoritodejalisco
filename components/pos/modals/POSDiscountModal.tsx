...
export function POSDiscountModal({
  ...
  return (
    ...
    >
      <div className="bg-card rounded-xl max-w-md w-full p-5 shadow-2xl border border-border space-y-4 max-h-[92vh] overflow-y-auto custom-scrollbar">
        {/* Header */}
        <div className="flex items-start justify-between border-b border-border pb-3">
          <div className="space-y-0.5">
            <h2
              id="discount-modal-title"
              className="text-sm sm:text-base font-bold text-text-light uppercase tracking-tight flex items-center gap-2"
            >
          ...
          <button
            type="button"
            onClick={onClose}
            className="text-text-light/40 hover:text-text-light p-1.5 rounded-lg hover:bg-white/10 transition-colors cursor-pointer"
            aria-label="Cerrar"
          >
            <X className="h-4 w-4" />
          </button>
        ...
        <form onSubmit={handleApplyClick} className="space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            >
              <button
                ...
                className={`flex items-center justify-center gap-2 py-2 rounded-lg text-xs font-bold uppercase transition-all border outline-none cursor-pointer active:scale-[0.98] ${
                  type === "PERCENT"
                    ? "bg-primary/20 border-primary text-primary shadow-sm"
                    : "bg-white/5 border-border/40 text-text-light/60 hover:border-border hover:text-text-light"
                }`}
              >
              ...
              <div className="grid grid-cols-5 gap-1.5">
                {DISCOUNT_PERCENT_PRESETS.map((pct) => (
                  <button
                    ...
                    className={`py-1.5 text-xs font-mono font-bold rounded-lg border transition-all cursor-pointer active:scale-[0.98] ${
                      numericValue === pct
            ...
            <div className="space-y-1.5 p-3 rounded-lg bg-secondary border border-border">
              ...
              <div className="grid grid-cols-2 gap-2">
                <button
                  ...
                  className={`py-1.5 px-2 text-xs font-bold rounded-md border transition-all cursor-pointer text-center ${
                    scope === "ROW"
                      ? "bg-amber-500/20 border-amber-500 text-amber-400"
                      : "bg-white/5 border-border/40 text-text-light/60 hover:text-text-light"
                  }`}
                >
                  Total Renglón
                </button>
          ...
          <div className="flex flex-col gap-2 pt-2">
            <button
              ...
              className="w-full bg-primary text-background py-2.5 rounded-lg font-bold text-xs hover:brightness-110 active:scale-[0.98] shadow-sm focus-visible:ring-2 focus-visible:ring-primary outline-none disabled:opacity-30 disabled:pointer-events-none transition-all uppercase tracking-wider flex items-center justify-center gap-1.5 cursor-pointer"
            >
            ...
            {initialDiscount?.discountType && (
              <button
                ...
                className="w-full bg-red-500/10 text-red-400 border border-red-500/20 py-2 rounded-lg font-bold text-xs hover:bg-red-500/20 hover:border-red-500/30 active:scale-[0.98] transition-all uppercase tracking-wider flex items-center justify-center gap-1.5 cursor-pointer"
              >
            ...
            <button
              type="button"
              onClick={onClose}
              className="w-full bg-white/5 text-text-light/60 py-2 rounded-lg font-bold text-xs hover:bg-white/10 hover:text-text-light transition-all uppercase tracking-wider cursor-pointer"
            >
              Cancelar
            </button>
          </div>
