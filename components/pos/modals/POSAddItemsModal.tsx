...
export function POSAddItemsModal() {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center p-2 sm:p-4 z-50 no-print">
      <div className="bg-card rounded-xl max-w-4xl w-full h-[92vh] md:h-[82vh] shadow-2xl border border-border flex flex-col md:flex-row overflow-hidden">
        {/* =========================================================================
        ...
        >
          ...
          <div className="p-3 sm:p-4 border-b border-border flex items-center justify-between gap-3 bg-card shrink-0">
            <div>
              <h3 className="text-sm sm:text-base font-bold text-text-light uppercase tracking-tight flex items-center gap-2">
                <span className="h-2 w-2 rounded-full bg-primary animate-pulse" />
                Agregar a Orden #{editingOrder.orderNumber}
              </h3>
              {(editingOrder.table || editingOrder.customer?.name) && (
                <p className="text-[11px] font-semibold text-text-light/50 uppercase tracking-wider mt-0.5">
            ...
            <button
              ...
              className="text-text-light/40 hover:text-text-light p-1.5 rounded-lg hover:bg-white/10 transition-colors disabled:opacity-30 cursor-pointer"
              aria-label="Cerrar"
            >
              <X className="h-4 w-4" />
            </button>
            ...
            <div className="relative">
              ...
              {searchQuery && (
                <button
                  ...
                  className="absolute right-2 top-1/2 -translate-y-1/2 h-6 w-6 flex items-center justify-center rounded-md text-text-light/40 hover:text-text-light hover:bg-white/10 transition-colors cursor-pointer"
          ...
          <div className="px-3 py-2 border-b border-border flex gap-1.5 overflow-x-auto no-scrollbar shrink-0 bg-card">
            <button
              type="button"
              onClick={() => setActiveCategory("")}
              className={`px-3 py-1 rounded-md font-bold text-[11px] uppercase tracking-wider transition-all whitespace-nowrap border cursor-pointer ${
                activeCategory === ""
                ...
                {filteredMenuItems.map((m) => {
                  ...
                  return (
                    <button
                      ...
                      disabled={isSubmittingCart}
                      className={`group relative rounded-xl bg-card p-3 border transition-all shadow-sm flex flex-col justify-between text-left h-24 overflow-hidden active:scale-[0.98] cursor-pointer disabled:opacity-50 disabled:pointer-events-none ${
                        isOutOfStock
                    ...
                    >
                      ...
                      <div className="absolute top-2 right-2 flex items-center gap-1 z-10">
                        ...
                        {isOutOfStock && (
                          <span className="rounded bg-red-500/20 border border-red-500/30 px-1 py-0.5 text-[8px] font-bold uppercase tracking-wider text-red-400">
                            Sin Stock
                          </span>
                        )}
                        {isLowStock && !isOutOfStock && (
                          <span className="rounded bg-amber-500/20 border border-amber-500/30 px-1 py-0.5 text-[8px] font-bold uppercase tracking-wider text-amber-400">
                            Bajo Stock
                          </span>
                        )}
                      </div>

                      <span className="text-xs font-semibold text-text-light line-clamp-2 pr-12 leading-snug">
                        {m.name}
                      </span>
                      ...
                      <div className="flex items-center justify-between w-full mt-2">
                        ...
                        <div className="h-6 w-6 rounded-md bg-white/5 group-hover:bg-primary/20 group-hover:text-primary flex items-center justify-center text-text-light/50 transition-colors">
          ...
          <div className="p-3 border-t border-border bg-card md:hidden shrink-0">
            <button
              ...
              className="w-full bg-primary text-background py-2.5 px-4 rounded-lg font-bold text-xs uppercase tracking-wider flex items-center justify-between shadow-sm active:scale-[0.98] transition-all cursor-pointer"
            >
        ...
        >
          ...
          <div className="p-3 sm:p-4 border-b border-border flex items-center justify-between gap-2 shrink-0 bg-card">
            <div className="flex items-center gap-2">
              {showMobileTray && (
                <button
                  ...
                  className="md:hidden text-text-light/60 hover:text-text-light p-1 rounded-md hover:bg-white/10 transition-colors cursor-pointer"
            ...
            <span className="text-[11px] font-mono font-bold px-2 py-0.5 rounded-md bg-primary/10 text-primary border border-primary/20 tabular-nums">
                ...
                >
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      ...
                      <p className="text-[11px] font-mono font-medium text-text-light/50 tabular-nums">
                        ${item.unitPrice.toFixed(2)} c/u
                      </p>
                    </div>

                    <div className="text-right shrink-0">
                      <span className="text-xs font-mono font-bold text-text-light tabular-nums">
                  ...
                  <div className="flex items-center justify-between gap-2 pt-1">
                    <div className="flex items-center gap-1 bg-dark/40 rounded-lg p-0.5 border border-border">
                      <button
                        ...
                        disabled={isSubmittingCart}
                        className="h-6 w-6 flex items-center justify-center rounded-md hover:bg-white/10 text-text-light/70 hover:text-text-light transition-colors disabled:opacity-30 cursor-pointer active:scale-95"
                      ...
                      <input
                        ...
                        className="w-10 text-center text-xs font-mono font-bold text-text-light bg-transparent outline-none [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none tabular-nums"
                        aria-label="Cantidad"
                      />
                    ...
                    <button
                      ...
                      disabled={isSubmittingCart}
                      className="text-red-400/60 hover:text-red-400 hover:bg-red-500/10 p-1.5 rounded-md transition-colors disabled:opacity-30 cursor-pointer"
          ...
          <div className="p-3 sm:p-4 border-t border-border bg-card space-y-3 shrink-0">
            <div className="flex items-center justify-between text-xs font-bold uppercase tracking-wider text-text-light">
            ...
            <div className="flex gap-2">
              <button
                ...
                disabled={isSubmittingCart}
                onClick={() => setEditingOrder(null)}
                className="w-1/3 bg-white/5 text-text-light/60 py-2.5 rounded-lg font-bold border border-border hover:bg-white/10 hover:text-text-light transition-all uppercase text-xs tracking-wider disabled:opacity-50 cursor-pointer"
              >
                Cancelar
              </button>
              ...
              <button
                ...
                className="w-2/3 bg-primary text-background py-2.5 px-3 rounded-lg font-bold hover:brightness-110 active:scale-[0.98] transition-all uppercase text-xs tracking-wider shadow-sm disabled:opacity-30 disabled:pointer-events-none truncate cursor-pointer"
              >
                {isSubmittingCart
                  ? "Guardando..."
                  : trayItems.length > 0
