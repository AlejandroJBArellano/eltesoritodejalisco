...
describe("OrderDetailExpanded Component", () => {
  ...
  it("renders audit timeline with all event types and badges", () => {
    ...
    expect(screen.getAllByText("Productos agregados").length).toBeGreaterThan(
      0,
    );
    expect(screen.getAllByText("Productos eliminados").length).toBeGreaterThan(
      0,
    );
    ...
    expect(
      screen.getByText(/Papulince comandó: Torta Ahogada x3, Horchata x2/),
    ).toBeDefined();
    expect(screen.getByText(/Mesero Juan agregó: Cerveza x1/)).toBeDefined();
    expect(
      screen.getByText(
        /Mamulince eliminó: Torta Ahogada x1 \(Autorizado con PIN\)/,
      ),
    ).toBeDefined();
    expect(
      screen.getByText(
        /Admin aplicó descuento: -10% \(Cortesía\) \(Autorizado con PIN\)/,
      ),
    ).toBeDefined();
    expect(screen.getByText(/Admin aplicó descuento: -\$25.00/)).toBeDefined();
    expect(
      screen.getByText(/Carlos registró cobro \(Efectivo\)/),
    ).toBeDefined();
    expect(
      screen.getByText(
        /Gerente Laura reabrió la cuenta: Error en propina \(Autorizado con PIN\)/,
      ),
    ).toBeDefined();
    expect(
      screen.getByText(
        /Admin canceló la orden \(Cancelación definitiva\) \(Autorizado con PIN\)/,
      ),
    ).toBeDefined();
    expect(
      screen.getByText(/Sistema modificó la comanda \(Autorizado con PIN\)/),
    ).toBeDefined();
  ...
  it("does not show '(Autorizado con PIN)' when user is Admin or same user as authorizedBy", () => {
    ...
    expect(
      screen.getByText(/Alejandro Arellano eliminó: papulince x4/),
    ).toBeDefined();
    // No debe contener el sufijo de PIN
    expect(
      screen.queryByText(/papulince x4 \(Autorizado con PIN\)/),
    ).toBeNull();

    // Debe limpiar el sufijo redundante 'eliminado(s)' del resumen antiguo
    expect(
      screen.getByText(/Alejandro Arellano eliminó: 1 producto\(s\)$/),
    ).toBeDefined();
